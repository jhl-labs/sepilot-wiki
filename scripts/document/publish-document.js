...
async function main() {
  ...
  try {
    ...
    // wiki-maintenance 이슈는 발행 처리 건너뛰기
    if (context.labels?.includes('wiki-maintenance')) {
      console.log('⏭️ wiki-maintenance 이슈는 문서 발행을 건너뜁니다.');
      return;
    }
    ...
    await removeLabel(issueNumber, 'draft');
    await addLabels(issueNumber, ['published']);
