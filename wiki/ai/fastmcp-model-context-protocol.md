...
## 5. 설치 및 환경 설정
### 지원되는 Python 버전 및 의존성  
- Python **3.9 이상** (정확한 버전은 프로젝트 `pyproject.toml` 에 명시)  
- 주요 의존성: `fastmcp`, `httpx`, `pydantic` 등 (패키지 설치 시 자동 해결)
...
### 로컬 개발 환경 구성 팁  
- 가상 환경(`venv` 혹은 `conda`)을 사용해 의존성을 격리  
- `uvicorn` 등 ASGI 서버와 연동해 비동기 모드 테스트 가능 (FastMCP 자체 서버 실행 시 자동 관리)  
