...
1. **토큰 검증** – 클라이언트가 전송한 JWT(또는 Casdoor‑발급 OAuth 토큰)를 검증하고, 토큰에 포함된 `exp`, `nbf`, `iss` 등을 확인한다.  
...
4. **감사 로그** – 허용·거부 여부, 사용자 ID, 요청 파라미터, 실행 시간 등을 **Self‑Logging Architecture**(섹션 4.7)와 연계해 기록한다.  
...
MCP는 **OAuth 2.0 Scopes**와 유사한 방식으로 권한을 선언한다. 주요 포인트는 다음과 같다.
...
| **Scope 검증** | 서버는 토큰의 `scope` 클레임을 파싱하고, 요청된 `toolId`·`resourceId`와 매칭한다. 불일치 시 `403` 반환. | 위 섹션 4.9.4 |
| **동적 스코프** | 토큰 발급 시점에 **조건부** 스코프를 부여할 수 있다(예: 특정 세션 ID와 연결). | Logto 블로그(2025‑06‑18) |
...
# 예시 비밀키 (프로덕션에서는 Vault/Secrets Manager 사용)
...
class AuthorizationGateway(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # 4️⃣ 스코프 매핑 (예: tool_name -> required scope)
        required_scope = {
...
1. **Authorization Header** 로 JWT를 받아 `verify_token` 로 검증.  
2. **Scope** 클레임을 파싱하고, 요청된 Tool에 매핑된 **필수 스코프**와 비교.  
3. **역할(RBAC)** 정책(`POLICIES`)과 교차 검증.  
4. 검증 성공 시 `request.state.auth` 에 메타데이터를 저장하고, 이후 Tool 핸들러가 이를 활용해 추가 로깅·감사를 수행한다.  
5. 검증 실패 시 즉시 `401` 혹은 `403` 응답을 반환한다.  
...
| **외부 볼륨** | Kubernetes `PersistentVolumeClaim` 혹은 Docker `volume`에 `/data` 마운