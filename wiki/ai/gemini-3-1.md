...
## 5. LlamaParse 소개 및 Gemini 3.1과의 통합
...
## 14. Regional Rollout (India, Canada, New Zealand)
...
## 16. Upcoming Event – Google I/O 2026
...
## 17. Keynote & Panel Highlights – Gemini AI 관련 발표 요약
...
## 18. Anticipated Gemini Features – 발표된 신규 모델·API·통합 내용
...
## 19. Google TV 통합 (신규 섹션)

Google TV에 Gemini를 통합함으로써 TV 사용자에게도 AI 기반 멀티모달 경험을 제공한다. 2026년 CES에서 공개된 기능들은 현재 미국에서 순차 롤아웃 중이며, 캐나다와 기타 지역에서는 차례로 제공될 예정이다.

### 19.1 Richer Visual Help

- **시각적 답변 강화**: 텍스트 질문에 대해 관련 이미지·동영상·애니메이션을 함께 제공한다.  
- **예시**: 레시피를 요청하면 Gemini가 단계별 동영상 튜토리얼을 재생한다.  
- **스코어카드**: 경기 점수를 물어보면 실시간 스코어와 해당 경기를 시청할 수 있는 채널 정보를 표시한다.

### 19.2 Deep Dives

- **교육용 시각적 설명**: 사용자가 “Deep dive”를 선택하면 Gemini가 내레이션이 포함된 애니메이션·다이어그램을 생성해 주제를 상세히 설명한다.  
- **후속 질문**: 학습 흐름을 이어가기 위해 자동으로 관련 후속 질문을 제시한다.  
- **접근 방법**: Gemini 탭에서 “Learn”을 선택하거나, 질문 후 “Dive deeper” 버튼을 클릭한다.

### 19.3 Sports Briefs

- **실시간 스포츠 뉴스 요약**: NBA, NCAA 농구, NHL, MLB, MLS, NWSL 등 주요 리그의 최신 소식을 내레이션과 함께 제공한다.  
- **초기 제공 지역**: 미국에서 바로 이용 가능. 캐나다에서는 Richer Visual Help만 먼저 제공되며, Deep Dives와 Sports Briefs는 봄에 순차 롤아웃 예정이다.
...
- **동영상·이미지 튜토리얼**: 레시피, DIY, 코딩 등 다양한 도메인에서 실시간 동영상 가이드를 제공한다.  
- **실시간 스코어카드 UI**: 경기 진행 상황을 그래픽 오버레이 형태로 표시하고, 사용자가 선택하면 해당 스트리밍 채널로 바로 이동한다.  
- **음성 비서 연동**: “Hey Google, 오늘 NBA 경기 결과 알려줘”와 같은 음성 명령으로 Sports Briefs를 호출한다.

---

## 20. Personal Intelligence 기능 확대

2026년 3월 17일, Google은 **Personal Intelligence** 기능을 미국 전역의 모든 무료 사용자에게 확대했다. 기존에는 Google AI Pro 및 AI Ultra 구독자에게만 제한적으로 제공되었지만, 이제 **AI Mode in Search**, **Gemini 앱**, **Chrome**에서 무료로 이용할 수 있다.

- **대상**: 미국에 있는 개인 Google 계정 사용자(무료·프리미엄 모두)  
- **제외**: Workspace 비즈니스, 엔터프라이즈, 교육용 계정은 아직 지원되지 않음  
- **배포 시점**: 2026‑03‑17 발표 이후 순차 롤아웃 시작  
