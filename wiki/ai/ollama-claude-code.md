...
## 12. VS Code 플러그인 설치
VS Code에서 로컬 Ollama 모델을 직접 활용하려면 **Continue** 확장 프로그램을 설치합니다.

1. VS Code를 실행합니다.  
2. **Extensions** 뷰 (`Ctrl+Shift+X`) 로 이동합니다.  
3. 검색창에 `Continue` 를 입력하고 **Install** 버튼을 클릭합니다.  

Continue는 Ollama와의 통합을 기본 제공하며, 설치 후 바로 설정 파일을 편집해 로컬 모델을 지정할 수 있습니다.

## 13. Ollama 모델 선택 및 실행
코딩 작업에 적합한 모델은 `qwen3-coder:7b` 입니다. 모델을 실행하고 테스트하는 방법은 다음과 같습니다.

```bash
# 모델 실행 (첫 실행 시 자동 다운로드)
ollama run qwen3-coder:7b
```

또는 GUI(데스크톱 앱)에서 모델을 선택해 테스트할 수 있습니다. 모델이 정상적으로 로드되면 VS Code 내 Continue 창에서 질문을 입력해 결과를 확인합니다.

## 14. 디버깅 및 워크플로 팁
| 상황 | 팁 |
|------|----|
| **모델 응답이 느릴 때** | `ollama serve` 를 백그라운드에서 실행하고, CPU/메모리 사용량을 `htop` 등으로 모니터링합니다. 필요 시 `--num_gpu` 옵션으로 GPU 사용을 활성화합니다. |
| **특정 파일·디렉터리만 분석하도록** | Continue 설정(`~/.continue/config.json`)에 `includePaths` 혹은 `excludePaths` 옵션을 추가해 범위를 제한합니다. |
| **오류 로그 확인** | Ollama 로그는 터미널에 출력됩니다. `ollama serve --log-level debug` 로 상세 로그를 확인하고, 문제가 지속되면 `ollama stop && ollama serve` 로 재시작합니다. |
| **다중 모델 전환** | 여러 모델을 동시에 사용하고 싶다면 `config.json`에 모델 배열을 정의하고, VS Code 명령 팔레트(`Ctrl+Shift+P`)에서 “Continue: Switch Model”을 선택합니다. |
| **프롬프트 품질 향상** | 코드베이스 전체를 설명하도록 할 때는 `@project` 태그를 사용해 전체 컨텍스트를 제공하고, 구체적인 파일명을 명시하면 더 정확한 답변을 얻을 수 있습니다. |

위 팁을 활용하면 VS Code와 Ollama 기반 로컬 AI 코딩 어시스턴트를 보다 안정적이고 효율적으로 운영할 수 있습니다.