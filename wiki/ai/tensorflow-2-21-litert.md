...
| **3. LiteRT delegate 로드** | `lite_rt_delegate = tf.lite.experimental.load_delegate('lite_rt')`<br>`interpreter = tflite.Interpreter(model_path="model.tflite", experimental_delegates=[lite_rt_delegate])` | `interpreter.allocate_tensors` 성공 |
| **4. 기능 테스트** | 동일 입력에 대해 기존 TFLite와 LiteRT 출력 차이 < 1e‑5 (float) 혹은 정밀도 허용 범위 내 | `np.allclose(output_tflite, output_litert, atol=1e-5)` 통과 |
| **5. 성능 측정** | GPU·NPU·CPU 각각에 대해 latency/throughput 측정 후 목표치 달성 여부 확인 | GPU에서 **1.4×** 가속 확인 (벤치마크 기준) |
| **6. 최적화 재양자화** (선택) | `tf.lite.Optimize.DEFAULT` 로 재양자화 후 모델 크기·성능 개선 | 모델 파일 크