...
## 4. Single‑Call Cost Tracking Overview
고객별·기능별 AI 비용을 실시간으로 파악하려면 **AI 호출이 발생하는 순간** 비용 정보를 캡처하고, 해당 호출의 컨텍스트(고객 ID, 기능 키 등)를 함께 저장해야 합니다.  
이 접근 방식은 별도의 청구서 스크래핑이나 배치 ETL 없이도 **한 번의 HTTP 이벤트**만으로 비용 데이터를 수집할 수 있게 해 줍니다. 아래는 최근 euno.news에서 소개된 Tanso 기반 구현 예시와 핵심 필드를 정리한 내용입니다.

### 4.1 핵심 개념
- **eventIdempotencyKey** – 네트워크 재시도 시 중복을 방지하는 고유 키.  
- **customerReferenceId** – 서비스가 이미 보유하고 있는 고객 식별자.  
- **featureKey** – 비용을 연결할 기능(예: `ai_summarization`).  
- **costInput** – 모델·제공자 메타데이터.  
- **usageUnits** – 토큰 수 등 사용량 단위.  
- **costAmount** – 달러 기준 비용 (센트가 아니라 소수점 형태).

### 4.2 Implementation Example (REST)
...
### 4.3 Implementation Example (GraphQL)

```graphql
mutation RecordAiEvent($input: AiEventInput!) {
  recordAiEvent(input: $input) {
    success
    message
  }
}
...
### 4.4 Integration with Existing Monitoring Dashboards
- **Prometheus Exporter** – `TokenWatch`가 기록한 이벤트를 `/metrics` 엔드포인트에서 노출하면 Grafana 대시보드에 실시간 비용 그래프를 추가할 수 있습니다.  
- **Datadog / New Relic** – HTTP 이벤트를 전송한 뒤 동일한 payload를 로그 스트림에 복제하면 비용 메트릭을 기존 APM 파이프라인에 통합할 수 있습니다.  
- **Custom Dashboard** – `.tokenwatch/tokenwatch.log`를 ELK 스택에 파싱해 고객·기능별 비용 집계 시각화가 가능합니다.

### 4.5 Best‑Practice Checklist
- **Idempotency**: `eventIdempotencyKey`는 반드시 고유하게 생성하고, 재시도 시 동일 키를 재사용합니다.  
- **Secure Transmission**: API 호출은 HTTPS와 인증 토큰(`Authorization: Bearer <key>`)을 사용합니다.  
- **Accurate Cost Amount**: 비용은 달러 기준 소수점 형태(`0.05` = 5 ¢)로 전달합니다.  
- **Contextual Labels**: `customerReferenceId`와 `featureKey`를 모두 포함해 고객·기능별 집계가 가능하도록 합니다.  
- **Periodic Pricing Updates**: 모델 가격이 변동될 경우 `TokenWatch.update_pricing` 혹은 외부 가격 API와 연동해 최신 가격을 유지합니다.  
- **Monitoring Alerts**: 비용 임계값 초과 시 Slack/Email 알림을 설정해 재무 위험을 즉시 감지합니다.  
...
## 9. Budget Race Condition 및 동시성 안전 패턴
### 9.1 레이스 컨디션이 발생하는 이유
AI 에이전트 워크플로에서 예산을 확인하고 차감하는 순서를 단순히  
```python
...
if remaining >= estimated_cost:
    ...
    write_new_balance()
```  
와 같이 구현하면 **읽기‑쓰기 사이에 다른 스레드/프로세스가 예산을 변경**할 수 있습니다. 동시에 여러 호출이 진행되면 실제 사용량이 예산을 초과해도 차감 로직이 정상적으로 동작하지 않아 비용 초과가 발생합니다.

### 9.2 동시성‑안전 예산 관리 패턴
| 패턴 | 특징 | 적용 예시 |
|------|------|----------|
| **원자적 카운터 (Atomic Counter)** | 메모리 내에서 `threading.Lock` 혹은 `multiprocessing.Value` 로 보호 | ```python\nfrom threading import Lock\nbudget_lock = Lock()\n\ndef safe_spend(cost):\n    with budget_lock:\n        if get_remaining_budget() >= cost:\n            write_new_balance(cost)\n            return True\n        return False\n``` |
| **파일 기반 락** | 로컬 파일에 `filelock` 라이브러리 사용 → 다중 프로세스 간 동기화 | ```python\nfrom filelock import FileLock\nlock = FileLock('.tokenwatch/budget.lock')\n\ndef safe_spend(cost):\n    with lock:\n        if get_remaining_budget() >= cost:\n            write_new_balance(cost)\n            return True\n        return False\n``` |
| **SQLite 트랜잭션** | 단일 파일 DB를 이용해 ACID 보장. `BEGIN IMMEDIATE` 로 잠금 확보 | ```python\nimport sqlite3\nconn = sqlite3.connect('.tokenwatch/budget.db', isolation_level='IMMEDIATE')\n\ndef safe_spend(cost):\n    cur = conn.cursor()\n    cur.execute('SELECT remaining FROM budget')\n    remaining = cur.fetchone()[0]\n    if remaining >= cost:\n        cur.execute('UPDATE budget SET remaining = remaining - ?', (cost,))\n        conn.commit()\n        return True\n    conn.rollback()\n    return False\n``` |
| **분산 락 (Redis / etcd)** | 클러스터 환경에서 중앙 락 서비스 사용. TTL 로 교착 상태 방지 | ```python\nimport redis\nr = redis.StrictRedis()\n\ndef safe_spend(cost):\n    lock = r.lock('budget_lock', timeout=5)\n    if lock.acquire(blocking=True):\n        try:\n            remaining = int(r.get('budget_remaining'))\n            if remaining >= cost:\n                r.decrby('budget_remaining', cost)\n                return True\n            return False\n        finally:\n            lock.release()\n``` |

### 9.3 안전 예산 체크 코드 스니펫
아래 예시는 `TokenWatch`와 `filelock`을 결합한 구현입니다.

```python
import yaml
from filelock import FileLock
from tokenwatch import record

BUDGET_FILE = ".tokenwatch/budget.yaml"
LOCK_FILE = ".tokenwatch/budget.lock"
...
def safe_call_model(prompt: str, estimated_cost: float):
    lock = FileLock(LOCK_FILE)
    with lock:
        budget = load_budget()
        if budget["monthly"] < estimated_cost:
            raise RuntimeError("예산 초과: 호출을 중단합니다.")
        # 실제 모델 호출 (예: OpenAI)
        ...
        # 사용량 기록
        record(tokens=response.usage.total_tokens, label="agent-call")
        # 예산 차감
        budget["monthly"] -= estimated_cost
        save_budget(budget)
    return response
```
...
### 9.4 테스트 및 모니터링 권고사항
1. **동시성 테스트**  
   - `pytest` + `threading` 혹은 `multiprocessing` 로 10~100개의 동시 호출을 시뮬레이션.  
   - 모든 스레드가 `safe_spend` 를 호출한 뒤 최종 예산이 `initial - sum(costs)` 와 일치하는지 검증.

2. **재시도 시나리오**  
   - 네트워크 오류 등으로 인해 동일 호출이 재시도될 때 `eventIdempotencyKey` 로 중복 차감을 방지하도록 테스트.

3. **모니터링**  
   - `TokenWatch`가 기록한 `budget.yaml` 변화를 Prometheus `file_exporter` 로 노출하거나, 차감 이벤트를 별도 메트릭(`budget_spent_total`) 으로 카운트.  
   - 알림 임계값(예: 90 %) 도달 시 Slack/Email 알림을 설정하고, 대시보드에 “예산 소진 위험” 그래프를 추가.

4. **로그 검증**  
   - `.tokenwatch/tokenwatch.log`에 `budget_check` 와 `budget_update` 로그 레벨을 `INFO` 로 남겨, 운영 중에 언제 예산이 차감됐는지 추적.
...
| **동시성 레이스 컨디션이 발생합니다** | 9.2 절에서 소개한 원자적 카운터, 파일 락, SQLite 트랜잭션, 혹은 분산 락 중 하나를 적용해 보세요. |
