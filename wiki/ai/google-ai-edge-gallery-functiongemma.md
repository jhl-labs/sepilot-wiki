...
## 2. 온‑디바이스 함수 호출 개요 *(신규 섹션)*

FunctionGemma는 **270 M 파라미터**를 갖춘 특화 모델로, 자연어 명령을 **함수 시그니처와 매핑**하고 바로 실행하도록 설계되었습니다. 모델은 **오프라인 작동**을 전제로 하며, Google AI Edge Gallery와 LiteRT‑LM 런타임을 통해 모바일 디바이스에서 **액션‑지향** AI 경험을 제공합니다. 주요 특징은 다음과 같습니다.

- **오프라인 실행**: 클라우드 연결 없이 복잡한 작업을 수행해 높은 속도와 낮은 지연 시간을 제공한다.  
- **액션‑지향 함수**: 캘린더 관리, 디바이스 하드웨어 제어, 게임 로직(예: “Tiny Garden” 데모) 등 실제 행동을 트리거한다.  
- **Google AI Edge & LiteRT‑LM 통합**: 경량 런타임이 모델 압축·양자화와 하드웨어 가속을 담당한다.  
- **테스트용 제공**: Android와 iOS용 Edge Gallery 앱에서 테스트용으로 제공되며, 개발자는 이를 활용해 텍스트 생성 수준을 넘어 실제 세계 행동을 수행하는 애플리케이션을 구축할 수 있다[[EUNO.NEWS](https://euno.news/posts/ko/on-device-function-calling-in-google-ai-edge-galle-ae4c94)]].

## 3. FunctionGemma 모델 소개
...
아래 예시는 FunctionGemma가 실제 디바이스 동작을 트리거하는 방식을 보여줍니다. 반환값은 JSON 형태이며, 앱 레이어에서 파싱해 해당 로직을 수행합니다.
...
이와 같이 FunctionGemma는 **액션‑지향** 함수를 통해 물리적·디지털 세계와 상호작용하는 에이전시 애플리케이션을 구현할 수 있습니다.

## 5. LiteRT‑LM을 활용한 온‑디바이스 실행

### LiteRT‑LM 개요 및 역할  
LiteRT‑LM은 Google AI Edge에서 제공하는 **경량 런타임**으로, 모델 압축·양자화와 하드웨어 가속을 담당합니다. FunctionGemma와 결합하면 메모리 사용량을 크게 줄이면서도 높은 추론 속도를 유지할 수 있습니다[[Google Developers Blog](https://developers.google.com/edge/gallery/literuntime)].
...
## 6. Google AI Edge Gallery와 통합 방법
...
## 7. 성능 및 배포 고려사항 *(신규 섹션)*

### 현재 알려진 벤치마크  
Google 공식 블로그에서는 **“낮은 지연 시간 및 높은 속도”**를 강조하고 있으나, 디바이스별 구체적인 수치는 공개되지 않았습니다[[EUNO.NEWS](https://euno.news/posts/ko/on-device-function-calling-in-google-ai-edge-galle-ae4c94)].
...
- **양자화 레벨**을 8‑bit으로 고정하고, 필요 시 **mixed‑precision** 옵션 검토  

### 배포 시 고려사항  
1. **모델 무결성 검증** – SHA‑256 서명 검증을 통해 변조 여부 확인.  
2. **버전 관리** – 모델 파일에 버전 태그(`functiongemma_v1.0.tflite`)를 부여하고, 앱 시작 시 최신 버전 여부를 체크.  
3. **하드웨어 가속 활용** – NPU/DSP 지원 디바이스에서는 LiteRT‑LM이 자동으로 가속 경로를 선택합니다.  
4. **오프라인 테스트** – 네트워크 차단 환경에서 전체 함수 호출 흐름을 검증하여 실제 배포 시 안정성을 확보합니다.
