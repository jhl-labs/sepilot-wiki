...
6. **MoD‑DPO 논문** – euno.news 요약 및 arXiv 2603.03192v1 (2026‑03‑03). <https://euno.news/posts/ko/mod-dpo-towards-mitigating-cross-modal-hallucinati-54c1e5>  
7. 기타 Hacker News, arXiv, 기업 백서 등 (구체 URL 미공개, 추가 조사 필요).  
...
### 13.1 연구 배경
Omni‑modal LLM(텍스트·이미지·오디오) 은 시청각 이해 작업에서 뛰어난 성능을 보이지만, **교차‑모달 환각**(입력 모달리티와 불일치하는 텍스트 생성) 문제가 여전히 존재한다. 기존 DPO(Direct Preference Optimization) 기반 파인튜닝은 텍스트‑중심 편향을 완화하지만, 멀티모달 정합성에는 한계가 있다.

### 13.2 핵심 기여
| 기여 | 설명 |
|------|------|
| **Modality‑Decoupled Direct Preference Optimization (MoD‑DPO)** | 표준 DPO 손실에 **모달리티 인식 정규화**를 추가한 파인튜닝 프레임워크. |
| **Invariance & Sensitivity Regularization** | *Irrelevant‑Modality Invariance*: 비관련 모달리티가 손상돼도 로짓이 변하지 않도록 함.<br>*Relevant‑Modality Sensitivity*: 관련 모달리티에 약간의 교란을 주면 로짓이 비례적으로 변하도록 강제, 진정한 grounding 촉진. |
| **Language‑Prior Debiasing Penalty** | 텍스트‑전용 프롬프트에 대해 언어 사전 지식이 과도하게 작용하는 경우 벌점을 부여, “텍스트‑전용 지름길”을 억제. |
| **Scalable Design** | 기존 omni‑LLM(예: Flamingo‑2, LLaVA‑Video) 구조를 변경하지 않고, 추가 PyTorch 모듈 형태로 바로 적용 가능. |
| **Empirical Validation** | 동일 연산 예산(≈ 2‑3 k GPU‑h)에서 AVQA‑Hallucination, Video‑Storytelling, Multimodal NLI 등 3개 벤치마크의 환각 비율을 40‑50 % 감소시키며 정답 정확도도 향상. |

### 13.3 실험 결과 (핵심 지표)
| 벤치마크 | Baseline DPO 환각 비율 | MoD‑DPO 환각 비율 | Perception Accuracy ↑ |
|----------|----------------------|-------------------|----------------------|
...
### 13.4 실용적 함의
- **신뢰성 향상**: 멀티모달 챗봇(비디오 지원 에이전트, 오디오 안내 편집기)에서 응답이 실제 미디어와 일치하도록 보장, 사용자 불만 감소.  
- **안전·규제 준수**: 의료 영상·자율 주행 등 규제 분야에서 “환각 없음”을 검증 가능한 방법 제공.  
- **비용 효율**: 기존 모델 위에 몇 에포크만 학습하면 되므로 대규모 사전학습 비용 없이도 성능 개선 가능.  
- **툴링 통합**: PyTorch 모듈 형태로 제공되어 🤗 Transformers 등 기존 파이프라인에 손쉽게 삽입 가능[[Makebot.ai](https://www.makebot.ai/blog/2026nyeoneul-iggeul-10gaji-haegsim-ai-llm-sijang-teurendeu)].  

### 13.5 제한 사항 및 향후 연구
- **Preference Data Dependency**: 고품질 선호 쌍이 필요하며, 도메인별 데이터 구축 비용이 존재.  
- **Modality Scope**: 현재는 텍스트·이미지·오디오에 초점; 깊이·3‑D·센서 데이터 등으로 확장 필요.  
- **Perturbation Design**: 교란 강도가 모델 성능에 미치는 영향이 민감하므로 자동 커리큘럼 학습이 제안됨.  
- **Long‑Form Consistency**: 짧은 QA·캡션에서는 개선되지만, 긴 서사·다중 턴 대화에서의 모달리티 정합성 유지는 추가 연구 필요.  

> MoD‑DPO는 2026년 LLM 연구 흐름에서 **멀티모달 환각 완화**라는 중요한 과제를 해결하는 최신 접근법으로, 향후 에이전시·RAG 파이프라인에 널리 적용될 전망이다.  