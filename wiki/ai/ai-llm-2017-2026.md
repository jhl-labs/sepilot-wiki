...
author: SEPilot AI
status: published
tags: [LLM, AI History, Timeline, Large Language Models, 기술 문서]
quality_score: 84
...
- **기존 자료와 차별점**: 기존 논문·블로그는 개별 모델에 초점을 맞추는 경우가 많다. 본 문서는 **전체 흐름**을 연대별·기술별·산업별로 구조화하고, 2025‑2026년 최신 에이전시·영구 기억·데이터‑컴퓨팅 스케일링 트렌드까지 포함한다.
...
| **2026** | **NanoGPT Slowrun** 발표 – 제한된 데이터(1억 토큰)와 사실상 무한 컴퓨팅을 활용한 데이터 효율적 학습 알고리즘 소개[[euno.news](https://euno.news/posts/ko/nanogpt-slowrun-language-modeling-with-limited-dat-35aa20)]. |
...
|      | **MoD‑DPO (paper)** | Modality‑Decoupled Direct Preference Optimization을 통해 옴니‑모달 LLM의 교차‑모달 환각을 40‑50% 감소시킴; Invariance & Sensitivity 정규화와 Language‑Prior Debiasing Penalty 도입[[euno.news](https://euno.news/posts/ko/mod-dpo-towards-mitigating-cross-modal-hallucinati-54c1e5)]. |
|      | **NanoGPT Slowrun** | 제한된 데이터(100 M 토큰)와 무제한 컴퓨팅 환경에서 검증 손실 최소화에 초점; 기존 modded‑nanogpt 대비 5.5× 데이터 효율성 달성[[euno.news](https://euno.news/posts/ko/nanogpt-slowrun-language-modeling-with-limited-dat-35aa20)]. |
...
|      | **MoD‑DPO (논문)** | 멀티모달 LLM의 교차‑모달 환각을 크게 감소시키는 경량 파인튜닝 레시피 발표. 상세 내용은 섹션 13에 정리. |
...
| **환각 완화 기술** | 2026년 발표된 **MoD‑DPO**는 멀티모달 LLM의 교차‑모달 환각을 감소시키는 정규화 기반 파인튜닝 기법으로, Invariance & Sensitivity 정규화와 Language‑Prior Debiasing Penalty를 결합한다. 동일 연산 예산에서 기존 DPO 대비 환각 비율을 40‑50% 낮추며, 다양한 시청각 벤치마크에서 정답 정확도도 향상시킨다[[euno.news](https://euno.news/posts/ko/mod-dpo-towards-mitigating-cross-modal-hallucinati-54c1e5)]. |
| **Compute vs. Data Scaling (NanoGPT Slowrun)** | 2026년 NanoGPT Slowrun은 **데이터는 1억 토큰으로 제한**하고 **컴퓨팅은 사실상 무제한**으로 활용해 손실 최소화에 집중한다. 이 접근은 “컴퓨팅은 데이터보다 훨씬 빠르게 성장한다”는 인사이트를 바탕으로, 데이터 효율성을 5.5배까지 끌어올렸다[[euno.news](https://euno.news/posts/ko/nanogpt-slowrun-language-modeling-with-limited-dat-35aa20)]. 또한, 다에폭 학습, 셔플링, SwiGLU 활성화 등 다양한 최적화 기법이 데이터 효율성을 크게 향상시켰다. |
...
| **기업 업무 자동화·에이전시** | GPT‑5·5.3, Gemini 3·3.5, MagicSuites(Makebot) | AI가 스스로 계획·도구 사용·업무 실행, 기업 프로세스 자동화 급증[[Makebot.ai](https://www.makebot.ai/blog/2026nyeoneul-iggeul-10gaji-haegsim-ai-llm-sijang-teurendeu)]. |
...
| **멀티모달 환각 완화** | MoD‑DPO (Flamingo‑2, LLaVA‑Video 기반) | 교차‑모달 환각 감소로 멀티모달 챗봇·비디오 요약·오디오 안내 등 서비스 신뢰도 향상. |
| **데이터‑컴퓨팅 스케일링** | NanoGPT Slowrun | 제한된 데이터(100 M 토큰)와 무제한 컴퓨팅으로 5.5× 데이터 효율성 달성, 새로운 학습 알고리즘 탐색 기반 연구 활성화[[euno.news](https://euno.news/posts/ko/nanogpt-slowrun-language-modeling-with-limited-dat-35aa20)]. |
...
- **멀티모달 환각**: MoD‑DPO와 같은 최신 연구는 환각을 감소시키는 기술적 방안을 제공하지만, 고품질 선호 데이터 확보와 정규화 설계가 여전히 과제로 남아 있다[[euno.news](https://euno.news/posts/ko/mod-dpo-towards-mitigating-cross-modal-hallucinati-54c1e5)]. |
- **Compute vs. Data Scaling**: NanoGPT Slowrun이 제시한 “데이터는 제한하고 컴퓨팅은 무제한” 접근은 기존 스케일링 법칙에 대한 비판적 재검토를 촉발하고 있다[[euno.news](https://euno.news/posts/ko/nanogpt-slowrun-language-modeling-with-limited-dat-35aa20)]. |
...
- **Compute‑Data 스케일링 연구**: NanoGPT Slowrun과 같은 데이터‑제한·컴퓨팅‑무제한 접근이 학계·산업에서 활발히 탐구될 것이며, 향후 “데이터 효율성 100배” 수준의 알고리즘이 등장할 가능성이 있다. |
...
6. **MoD‑DPO: Omni LLM에서 Cross‑modal Hallucinations를 완화하기 위한 Modality Decoupled Preference Optimization** – euno.news. <https://euno.news/posts/ko/mod-dpo-towards-mitigating-cross-modal-hallucinati-54c1e5> (arXiv 2603.03192v1, 2026‑03‑03)  
7. **NanoGPT Slowrun: 제한된 데이터와 무한 연산을 이용한 언어 모델링** – euno.news. <https://euno.news/posts/ko/nanogpt-slowrun-language-modeling-with-limited-dat-35aa20>  
8. 기타 Hacker News, arXiv, 기업 백서 등 (구체 URL 미공개, 추가 조사 필요).  
...
| 2023 | Falcon 180B | TII | 180 B | 오픈소스 고성능 |
...
| 2026 | MoD‑DPO (paper) | — | — | 교차‑모달 환각 40‑50% 감소 (Flamingo‑2, LLaVA‑Video 기반) |
| 2026 | NanoGPT Slowrun | — | — | 100 M 토큰 제한, 무제한 컴퓨팅, 5.5× 데이터 효율성[[euno.news](https://euno.news/posts/ko/nanogpt-slowrun-language-modeling-with-limited-dat-35aa20)]. |
...
| **MoD‑DPO** | Modality‑Decoupled Direct Preference Optimization, 멀티모달 LLM의 교차‑모달 환각을 감소시키는 파인튜닝 프레임워크 |
| **NanoGPT Slowrun** | 제한된 데이터(100 M 토큰)와 무제한 컴퓨팅을 활용해 손실 최소화에 초점 맞춘 학습 알고리즘 |

### 12.3 타임라인 시각화
> **※** 시각화 그래프(연도‑모델 수, 파라미터 규모 추이)는 별도 PNG 파일로 제공한다.  

## 13. 최신 연구 동향 – MoD‑DPO (2026)
**논문**: *MoD‑DPO: Omni LLM에서 Cross‑modal Hallucinations를 완화하기 위한 Modality Decoupled Preference Optimization* (arXiv 2603.03192v1, 2026‑03‑03)  
**출처**: euno.news 요약 및 원문[[MoD‑DPO 논문](https://euno.news/posts/ko/mod-dpo-towards-mitigating-cross-modal-hallucinati-54c1e5)]

*본 문서는 euno.news 타임라인, 2025‑2026년 최신 산업·보안·규제 자료, 그리고 2026년 발표된 MoD‑DPO 논문을 기반으로 작성되었으며, 공개되지 않은 구체 수치·세부 내용은 “추가 조사가 필요합니다”로 표시하였다.*