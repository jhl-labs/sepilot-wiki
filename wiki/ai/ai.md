...
### 21.1 패턴 소개
두‑에이전트 검토 패턴은 **에이전트 A**가 작업을 수행하고 **에이전트 B**가 그 결과를 독립적으로 검토하도록 설계합니다. 핵심은 **자기 검증을 금지**하고, 검토 에이전트가 원본 작업과 별도의 컨텍스트만을 사용한다는 점입니다. 이 패턴은 30일 이상 운영된 AI 시스템에서 나타나는 미묘한 실패 모드—에이전트가 기술적으로는 맞지만 실제 요구와는 다른 출력을 “성공”으로 보고하는 현상을 방지합니다【euno.news】.
...
2. **검토 에이전트(B)** 를 별도 프로세스로 배치하고, **동일 파일 시스템에 접근하되 **작업 사양**만 읽도록 제한합니다.  
3. **검토 결과**는 구조화된 JSON(`verdict`, `checked_at`, `notes`) 형태로 저장하고, **A**가 이를 파싱해 자동 재시도 로직을 트리거합니다.  
4. **CI/CD 파이프라인**에 `review.json` 존재 여부와 `verdict` 값을 검사하는 단계(`permission-check.yml`과 유사)를 추가해, `fail` 시 배포를 차단합니다.  
5. **모니터링**: `review.json` 생성·수정 이벤트를 로그에 기록하고, 실패 비율을 대시보드에 표시해 지속적인 품질 관리가 가능하도록 합니다.

### 21.6 권장 베스트 프랙티스
- **검토 에이전트는 별도 모델**(예: Claude vs. Copilot) 혹은 동일 모델의 다른 인스턴스를 사용해 **다양한 관점**을 확보합니다.  
- **검토 기준을 명문화**하고, `notes` 에는 가능한 한 구체적인 수정 지시를 포함합니다.  
- **검토 주기**를 자동화하여, 작업이 `pending_review` 상태에서 일정 시간 이상 머무르면 알림을 발생시킵니다.  
- **권한 최소화**: 검토 에이전트는 출력 파일을 읽고 검토 파일을 쓰는 권한만 부여하고, 코드베이스에 대한 쓰기 권한은 부여하지 않습니다.

## 22. 결론
멀티 AI 코딩 에이전트 오케스트레이션은 **작업 분해, 의존성 관리, 파일 잠금, 실시간 모니터링, 자동 복구**와 같은 핵심 요소를 결합함으로써 혼돈을 최소화합니다. 여기에 **콜드 스타트 방지**를 위한 **Day‑Zero 초기화 전략**, **두‑에이전트 검토 패턴**, **Verification Loop Prompt**, **Reverse‑CAPTCHA** 같은 보조 메커니즘을 추가