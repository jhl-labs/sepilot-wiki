...
### 5.1 런타임 LLM 금지 정책과 대안 설계
**출처**: [euno.news – 왜 우리는 Runtime에서 LLM을 금지했는가](https://euno.news/posts/ko/why-we-banned-llms-from-runtime-and-what-we-do-ins-b3c103)  

#### 5.1.1 금지 이유
| 문제 영역 | 현상 | 위험성 |
|-----------|------|--------|
| **비결정성** | 동일한 요청에 매번 다른 응답 | 예측 불가능한 비즈니스 로직 |
| **지연 시간** | 모델 부하에 따라 800 ms – 3 s | SLA 위반 및 사용자 경험 저하 |
| **비용** | 트래픽에 비례해 추론 비용이 선형 증가 | 비용 폭증 및 예산 초과 |
| **보안** | 모든 엔드포인트에 프롬프트 인젝션 표면 존재 | 악의적 입력을 통한 시스템 조작 |
| **감사 가능성** | “왜 이 값을 반환했나요?” → “모델이 그렇게 결정했어요.” | 책임 추적 및 규제 대응 어려움 |

프로토타입 단계에서는 허용될 수 있지만, 결제, 예약, 사용자 데이터 등을 처리하는 **프로덕션 백엔드**에서는 구조적인 위험이 됩니다.

#### 5.1.2 정책 적용 사례 – Fascia 접근법
- **설계 시점 AI**만 사용: 비즈니스 요구를 자연어로 기술하면 AI가 **구조화된 사양**(코드가 아니라)을 생성합니다. 사양에는 **Entities**, **Tools**, **Policies**가 포함됩니다.  
- **Deterministic Executor**: Go 로 구현된 실행 엔진이 사양을 읽고 실행합니다. 런타임에 LLM 추론이 없으며, 응답 시간은 **12 – 50 ms**(일관)이며 **비용이 발생하지 않음**.  
- **실행 흐름**: 입력 검증 → 인가 → 정책 확인 → 트랜잭션 시작 → 플로우 그래프 실행 → 불변 조건 적용 → 커밋/롤백 → 감사 로그 기록 → 타입된 응답 반환.  

| 메트릭 | LLM‑at‑runtime | Spec‑driven (Fascia) |
|--------|----------------|----------------------|
| 응답 시간 | 800 ms – 3 s (가변) | 12 – 50 ms (일관) |
| 결정론 | ❌ | ✅ |
| 프롬프트 인젝션 위험 | ⚠️ (모든 엔드포인트) | ❌ (런타임에 프롬프트 없음) |
| 요청당 비용 | 예 (추론 비용) | ❌ (추론 없음) |
| 감사 로그 | “모델이 결정” | 사양 버전 + 실행 로그 |

#### 5.1.3 프록시·캐시 기반 대안
1. **프록시 레이어**  
   - 클라이언트 요청을 프록시가 가로채고, 사양 기반 **캐시된 응답**을 반환합니다.  
   - 프록시는 사양이 변경될 때만 **새로운 실행**을 트리거하므로 추론 비용을 최소화합니다.  

2. **사전 생성된 응답 캐시**  
   - 설계 시점에 생성된 **정적 응답 집합**(예: FAQ, 정책 검증 결과)을 CDN에 배포하고, 런타임에서는 해당 캐시를 조회합니다.  

3. **Hybrid 모델**  
   - 고비용·고위험 트랜잭션은 **Spec‑driven** 경로로 강제하고, 비핵심 기능은 **LLM‑as‑proxy**를 사용하되 **Rate‑limit** 및 **Prompt‑whitelisting**을 적용합니다.  

#### 5.1.4 운영 가이드라인
- **설계 단계**  
  - 모든 비즈니스 로직을 **사양**(JSON/YAML)으로 정의하고, **Safety Agent**(멀티‑모델 교차 검증)로 검증한다.  
  - 위험 등급(Red) 패턴은 자동 차단하고, 설계 수정 없이 배포할 수 없게 한다.  

- **배포 전**  
  - 사양 검증 CI 파이프라인에 **정적 분석**(플로우 그래프, 불변 조건)와 **Policy Linter**를 포함한다.  
  - 정책 파일(`AGENTS.md`)에 **런타임 LLM 호출 금지** 규칙을 명시하고, 위반 시 CI 실패하도록 설정한다.  

- **런타임**  
  - **Deterministic Executor**만 실행하도록 인프라를 제한한다(예: Cloud Run, Cloud Functions).  
  - 모든 요청·응답은 **감사 로그**에 기록하고, 로그에는 사양 버전과 실행 메타데이터를 포함한다.  
  - **OpenTelemetry**를 이용해 비정상적인 API 호출 패턴을 실시간 모니터링하고, 알림을 설정한다.  

- **사후 관리**  
  - 사양 변경 시 **버전 관리**(Git)와 **자동 회귀 테스트**를 수행한다.  
  - 정기적인 **Security Review**와 **Compliance Audit**를 통해 정책 위반 여부를 확인한다.  

> 위 내용은 euno.news 기사에 기반한 사실이며, 추측이나 의견은 포함하지 않았습니다.
...
> **추가 조사 필요**: 현재 문서에서는 멀티‑에이전트 간 상호작용 테스트에 대한 구체적인 메트릭이 제시되지 않아, 해당 영역에 대한 베스트 프랙티스가 필요합니다.  
