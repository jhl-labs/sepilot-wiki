...
## 개요
이 문서는 **Kubernetes v1.36 (예정)** 에서 도입될 **Dynamic Resource Allocation (DRA)** 의 최신 변화와 실무 적용 방안을 미리 살펴봅니다.  

- **대상 독자**: 클러스터 운영자, 플랫폼 엔지니어, DevOps 팀, AI/ML 워크로드 담당자  
- **핵심 요약**  
  - v1.36은 아직 릴리즈되지 않았으며, 현재 **Alpha**·**Beta** 단계에 있는 여러 DRA 기능이 포함될 예정입니다.  
  - GPU·FPGA·ASIC 등 기존 가속기 외에 **네트워킹·스토리지** 등 이기종 하드웨어 지원이 확대됩니다.  
  - **Prioritized List**, **Native Resource Extension** 등은 현재 **Alpha** 단계이며, 향후 릴리즈 로드맵에 따라 단계가 승격될 가능성이 있습니다.  

> 참고: 현재까지 공개된 공식 자료는 Kubernetes Enhancement Proposals (KEP)와 v1.35 릴리즈 노트이며, v1.36에 대한 구체적인 릴리즈 일정은 아직 확정되지 않았습니다. (예: [KEP 2369 – Dynamic Resource Allocation](https://github.com/kubernetes/enhancements/tree/master/keps/sig-node/2369-dynamic-resource-allocation))
...
## v1.36에서 예정된 DRA 주요 업데이트
- **전체 흐름**: 현재 Alpha 단계에서 검증 중인 기능이 차례로 Beta·Stable 로 승격될 예정이며, 신규 드라이버와 네이티브 리소스 연계가 추가됩니다.  
- **릴리즈 로드맵(예정)**  

| 버전 | 단계 | 주요 내용 |
|------|------|-----------|
| v1.35 | Stable | 기존 DRA Alpha 기능(기본 ResourceClaim) 정식 GA |
| v1.36 | Alpha → Beta | Prioritized List, Native Resource Extension, ResourceClaims in PodGroups 등 신규 기능 도입 (현재 Alpha) |
| v1.37 | Beta → Stable | Alpha 단계 기능들의 승격 및 추가 하드웨어 드라이버 지원 확대 (예정) |

## Feature Graduations (기능 승격 현황)
| 기능 | 현재 단계 (v1.36 기준) | 설명 |
|------|----------------------|------|
...
- **지원 가속기**: GPU, FPGA, ASIC 외에 **SmartNIC**(네트워킹) 및 **NVMe 오프로드**(스토리지) 드라이버가 Alpha 단계에서 추가될 예정입니다.  
...
- **사용 시나리오**: 대규모 AI 학습 작업에서 여러 워커가 동시에 동일 GPU 풀을 사용해야 할 때, PodGroup + ResourceClaim 로 일관된 할당을 보장합니다.  
- **흐름**  
...
### 단계별 업그레이드 절차
1. **Control Plane** 업그레이드 → v1.36 알파 이미지 적용  
...
| 질문 | 답변 |
|------|------|
| **DRA와 기존 Extended Resource를 동시에 사용할 수 있나요?** | 네. v1.36 Alpha에서는 **Extended Resource Support (Beta)** 로 기존 선언을 그대로 유지하면서 DRA Claim 로 자동 전환이 가능합니다. |
...
- **릴리즈 노트 (예정)**: <https://github.com/kubernetes/kubernetes/releases/tag/v1.36.0> (예정 페이지)  
