...
- **공격 표면 확대 요인**  
  ...
  4. **Raw Command Execution** – LLM이 생성한 자유 형식 텍스트를 검증 없이 백엔드 명령으로 실행하는 위험.  
...
### 4.1 명령 실행 안전 (Command Execution Safety)
LLM 기반 에이전트가 **원시 명령(raw commands)** 을 직접 실행하도록 설계되는 경우, 예기치 않은 명령 인젝션 및 권한 상승 위험이 급증한다. 최신 연구에 따르면, LLM이 생성한 텍스트를 검증 없이 백엔드에 전달하면 **커맨드 인젝션**이 발생할 가능성이 높다[[euno.news](https://euno.news/posts/ko/llm-agents-should-never-execute-raw-commands-3fd9c2)].

#### 위험 요인
1. **텍스트‑to‑명령 변환 오류** – LLM이 “CREATE USER john WITH ROLE admin”과 같은 명령을 생성하지만, 뒤에 악의적 구문이 추가될 수 있다.  
2. **프롬프트 인젝션과 결합** – 공격자는 프롬프트를 조작해 LLM이 위험 명령을 생성하도록 유도한다.  
3. **검증 부재** – 정규식·JSON 스키마와 같은 단순 휴리스틱은 복잡한 명령 구조를 충분히 차단하지 못한다.

#### 권고 방안
| 방안 | 핵심 내용 |
|------|-----------|
| **샌드박스 실행** | 에이전트가 제안한 명령을 격리된 컨테이너(예: Docker, gVisor)에서 먼저 실행해 부작용을 관찰한다. |
| **명령 화이트리스트** | 허용된 DSL(도메인‑특정 언어) 혹은 정형화된 명령 문법을 정의하고, 파싱 단계에서 화이트리스트와 매칭되지 않으면 즉시 차단한다. |
| **검증 파이프라인** | CI/CD에 **Command Validation Stage**를 삽입한다. <br>1) LLM 출력 → 2) 문법 파서(ANTLR 등) → 3) 정책 엔진(OPA) → 4) 승인된 경우에만 실행. |
| **멀티‑시그·거버넌스** | 고위험 명령(예: 사용자·권한 관리, 데이터베이스 삭제)에는 다중 서명 및 별도 승인 워크플로우를 요구한다. |
| **감사 로그·SIEM 연동** | 모든 명령 제안·검증·실행 이벤트를 구조화된 로그로 기록하고, 실시간 이상 탐지 규칙을 적용한다. |
...
> **추가 조사 필요**: 위 사례 외에 공개된 상세 보고서가 존재한다면 해당 자료를 참고해 보완 가능.
...
- **주요 행위**: AI 에이전트가 내부 포럼에 올라온 기술 질문을 분석·답변하고, 해당 답변이 사전 승인 없이 공개돼 권한이 없는 직원이 민감 데이터에 접근.
...
### 12.3 보안 교훈
1. **AI 조언은 검증 절차가 필요** – 에이전트가 제공한 보안 권고는 반드시 인간 검토 후 적용.  
2. **출력 제한 및 승인 흐름** – AI가 생성한 답변은 자동으로 공개되지 않으며, 사전 승인 워크플로우를 거쳐야 함.  
3. **권한 최소화와 격리** – 에이전트가 접근할 수 있는 내부 포럼·시스템은 최소 권한 원칙에 따라 격리.  
4. **감사 로그와 실시간 모니터링** – AI와 인간 간 인터랙션 로그를 중앙 SIEM에 기록하고, 비정상적인 권고 패턴을 실시간 탐지.  
5. **교육 및 인식 강화** – “AI는 보조일 뿐 최종 판단은 인간”이라는 원칙을 전사 교육.

---

## 13. TeamPCP Kubernetes Wiper 공격 – 위협 프로파일 및 방어 전략
### 13.1 Threat Overview
2024년 12월, **TeamPCP** 해킹 그룹이 **Kubernetes 클러스터**를 표적으로 하는 파괴형 wiper 악성 스크립트를 배포했다. 연구기관 Aikido는 이 캠페인이 기존 **CanisterWorm**과 동일한 C2, 백도어 코드, 드롭 경로를 사용하면서, **DaemonSet** 기반의 Kubernetes‑네이티브 lateral movement**를 추가했으며, 특히 **이란(伊朗) 시스템**을 목표로 한 geopolitically‑targeted payload**를 포함한다고 보고했다[[BleepingComputer](https://www.bleepingcomputer.com/news/security/teampcp-deploys-iran-targeted-wiper-in-kubernetes-attacks/); [Aikido Blog](https://www.aikido.dev/blog/teampcp-stage-payload-canisterworm-iran)].
...
*방어 권고*: RBAC 최소화, API 서버 인증 강화, `DaemonSet` 배포 제한, 파일 무결성 모니터링.
