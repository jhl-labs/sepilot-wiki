...
## 10. 감사 결과 및 권고 조치
### 10.1 감사 개요
2024‑2025년 사이 **Botlington Agent Token Audit**를 10여 개 조직에 적용해 7‑턴 A2A(Agent‑to‑Agent) 인터뷰 방식으로 점수를 산정했다. 전체 평균 점수는 **62 / 100 (C+)**였으며, 주요 비효율·위험 요소가 다수 발견되었다[[euno.news](https://euno.news/posts/ko/what-we-found-when-an-ai-audited-an-ai-real-findin-b9a30d)].

### 10.2 발견된 주요 이슈
| 패턴 | 핵심 문제 | 예상 비용·리소스 오버헤드 | 권고 해결책 |
|------|-----------|--------------------------|-------------|
| **1. 기계적 작업에 잘못된 모델 사용** | 단순 패턴 매칭 작업에 고성능 LLM(GPT‑4, Claude Sonnet) 사용 | 40‑60 % 토큰 비용 증가 | - **Haiku**(경량 모델) → 기계적 조회<br>- **Sonnet** → 판단 필요 시만 사용<br>- **Opus** → 복합 합성 필요 시만 사용 |
| **2. 컨텍스트 부피 과다** | 매 실행마다 MEMORY.md(12 KB), TOOLS.md(4 KB), 프로젝트 브리프(8 KB) 등 전체 파일 로드 | 전체 컨텍스트 토큰 30‑50 % 초과 | - 작업에 필요한 파일만 로드<br>- 의미 검색 기반 메모리 저장소 활용<br>- 크론 작업 시 컨텍스트 윈도우 최소화 |
| **3. 도구 로딩 비효율** | 사용되지 않은 도구까지 로드 → 토큰·공격 표면 확대 | 도구 정의 토큰 10‑25 % 낭비 | - 작업에 맞는 도구 목록만 선언<br>- 불필요한 도구 제거 |
| **4. 상태 추적 부재** | 동일 이메일을 반복 처리 → 중복 작업 | 불필요한 실행 시간·토큰 소모 | - JSON 기반 **seen‑state** 파일(메시지 ID, 타임스탬프) 도입 |
| **5. 브라우저 자동화 남용** | API가 존재함에도 브라우저 자동화 사용 → 토큰 수십 배 증가 | 비용 급증, 공격 표면 확대 | - 먼저 API 존재 여부 확인<br>- API 사용이 불가능할 때만 브라우저 자동화 적용 |

### 10.3 비용·리소스 오버헤드 사례
- **월 평균 낭비**: €42 / 월 (11개 크론 작업)  
- **단일 작업 절감**: 모델 티어 최적화 시 40‑60 % 토큰 절감, 컨텍스트 부피 최적화 시 30‑50 % 절감, 불필요한 도구 로딩 시 10‑25 % 절감.  
- **감사 후 점수 상승**: 최적화 적용 후 평균 점수 **91 / 100** 달성, 월 비용이 크게 감소.

### 10.4 권고 조치
1. **모델 티어 정책 수립**  
   - 기계적 작업 → 경량 모델(Haiku) 사용  
   - 판단·창작 작업 → 고성능 모델(Sonnet/Opus) 사용  
2. **컨텍스트 관리**  
   - 작업별 최소 파일·데이터만 로드  
   - 의미 검색 기반 캐시 활용  
3. **도구 최소화**  
   - 에이전트 선언 파일에 실제 사용 도구만 포함  
4. **상태 추적 구현**  
   - 간단한 JSON 파일로 처리 이력 저장, 중복 실행 방지  
5. **API 우선 전략**  
   - 가능한 경우 브라우저 자동화 대신 공식 API 호출 사용  
6. **정기 감사 및 점수 모니터링**  
   - 월간/분기별 Botlington 스타일 감사 수행  
   - 점수 85 점 이상을 목표로 지속 개선  
