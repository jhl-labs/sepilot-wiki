...
## 4.5 AI 에이전트 지갑 보안 요구사항
AI 에이전트가 실제 금전적 가치를 다루는 경우, 기존 권한 과다 문제와 별도로 **지갑 설계**가 핵심 보안 경계가 된다. 아래 요구사항은 최신 라자루스(Lazarus) 공격 사례를 기반으로 정의한다.

### 4.5.1 라자루스 공격 시나리오
- **사건 개요**: 2024년 12월, 라자루스 그룹은 손상된 API 키와 고객용 AI 어시스턴트를 통한 **프롬프트 인젝션**을 결합해 Bitrefill의 핫 월렛을 탈취했다. 프롬프트 인젝션은 결제 백엔드에 대한 권한을 상승시켰으며, 중앙 관리되는 단일 키를 가진 핫 월렛은 몇 분 만에 비워졌다[[euno.news](https://euno.news/posts/ko/why-ai-agent-wallets-must-be-non-custodial-the-laz-59c9a1)].
- **핵심 특징**  
  1. **블록체인 취약점·스마트 계약 버그 없음** – 공격은 인프라 레이어(API 키·프롬프트)에서 발생.  
  2. **중앙 집중식 키**가 유일한 보호 장치였으며, 키가 유출되면 전체 잔액이 즉시 소진.  
  3. **수탁형(커스터디) 지갑**은 백엔드 명령 실행을 허용하므로, 프롬프트 인젝션이 직접적인 출금으로 이어진다.  
  4. **비수탁형(Non‑custodial) 지갑**은 온‑체인 지출 정책(예: ERC‑6551)으로 명령을 제한하고, 키 회전·자동 폐기가 가능해 동일 공격이 차단된다.

### 4.5.2 비수탁형 설계 권고사항
1. **온‑체인 지출 정책 적용**  
   - ERC‑6551 기반 **Token‑Bound Account**를 사용해 각 에이전트 지갑에 **SpendingPolicy**를 부여한다.  
   - 정책 예시: 거래당 최대 0.01 ETH, 일일 최대 0.1 ETH, 화이트리스트된 목적지 주소만 허용.  
   - 정책은 스마트 계약에 영구 저장되며, 계약 외의 모든 호출은 자동 거부된다.

2. **키 관리 최소화**  
   - 중앙 API 키를 절대 보관하지 않는다. 에이전트마다 **독립적인 비밀키**를 생성하고, 키 회전 주기를 짧게 설정한다.  
   - 손상된 키가 감지되면 **NFT 소유권 전송**(null 주소)으로 즉시 지갑을 비활성화한다.

3. **프롬프트 인젝션 방어**  
   - 입력 검증·샌드박스 레이어를 도입하고, AI 어시스턴트가 **결제 명령**을 생성하기 전 반드시 **정책 엔진**(OPA 등)과 교차 검증한다.  
   - 로그에 모든 프롬프트·응답을 기록하고, 이상 패턴 탐지를 위해 SIEM에 연동한다.

4. **목적지 화이트리스트와 한도**  
   - 결제 API 호출 시 **허용된 목적지 주소**만 전달하도록 강제한다.  
   - 온‑체인 한도 초과 시 자동 거부 및 알림을 발생시킨다.

5. **감사 및 모니터링**  
   - **AI‑to‑AI(A2A) 감사**를 정기적으로 실행해 지갑 정책 위반 여부를 점검한다.  
   - 실시간 알림을 Slack/Teams에 연결하고, 위반 시 OPA 정책으로 즉시 차단한다.

6. **인프라 최소 권한**  
   - 에이전트가 접근할 수 있는 API와 도구를 **필요 최소** 수준으로 제한한다.  
   - 다중‑에이전트 협업 시 각 에이전트에 별도 권한 프로파일을 적용한다.

#### 코드 예시 (TypeScript, ERC‑6551 기반)
```typescript
import { AgentWallet, SpendingPolicy } from '@ai-agent-economy/agent-wallet-sdk';
import { parseEther } from 'ethers';

// 비수탁형 에이전트 지갑 생성 – 강력한 가드레일 적용
...
```
*위 코드는 중앙 서버 없이 온‑체인 정책 검증만으로 결제를 수행한다는 점을 강조한다.*

---

## 5. AI‑to‑AI 감사 (A2A) 프레임워크
### 5.1 개념 소개
A2A(Agent‑to‑Agent) 감사는 **에이전트가 다른 에이전트를 직접 감사**하는 메커니즘으로, 인간 개입 없이 자연어 대화만으로 보안·컴플라이언스 평가를 수행한다. 2025년 4월 Google이 처음 도입한 이후, IBM 등 주요 클라우드 벤더가 개방형 프로토콜로 채택하고 있다[[IBM](https://www.ibm.com/think/topics/agent2agent-protocol)].
...
- **정책 엔진 연동**: OPA 등 정책 엔진에 권한 규칙을 정의하고, `/api/proxy`와 연동해 위반 시 차단.
...
- **프라이버시**: 감사 데이터는 최소 메타데이터만 포함하고, 민감 정보는 마스킹.  
...
6. **정기 감사** – 월/분기 Botlington 스타일 감사, 점수 85점 이상 목표.
...
| 클라이언트 힌트 주입 | `sec-ch-ua` 등 헤더를 실제 Chrome과 일치하도록 삽