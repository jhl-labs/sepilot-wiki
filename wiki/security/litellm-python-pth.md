...
스크립트는 다음과 같은 정보를 수집하고 외부 서버(구체적인 C2 URL은 `c2.teampcp.io`) 로 전송한다 [출처: euno.news]:
...
TeamPCP가 배포한 `1.82.7`·`1.82.8` 버전에는 다음과 같은 추가 기능이 포함된다:

| 기능 | 설명 |
|------|------|
| **자격 증명 수집기** | AWS, GCP, Azure 등 클라우드 제공자의 액세스 키·시크릿을 탐색·수집 |
| **쿠버네티스 횡방향 이동 툴킷** | 쿠버네티스 API 서버를 탐색하고 `pod‑to‑pod`, `node‑to‑node` 이동을 시도 |
...
| 방안 | 설명 |
|------|------|
| **Trivy 버전 고정 및 서명 검증** | CI에서 사용되는 Trivy 바이너리를 SHA‑256 해시와 서명으로 검증하고, 자동 업데이트를 차단한다. |
| **KICS 정책 강화** | KICS가 분석하는 IaC 파일에 대한 허용 목록을 적용하고, 외부 스크립트 로드(`*.pth`, `*.pyc`)를 차단한다. |
| **CI 이미지 검증** | 빌드 이미지에 포함된 모든 Python 패키지를 `trivy fs` 로 스캔하고, `.pth` 파일이 발견되면 빌드 실패하도록 설정한다. |
| **네트워크 차단** | `c2.teampcp.io` 로의 아웃바운드 트래픽을 방화벽 규칙으로 차단하고, DNS 쿼리 로깅을 활성화한다. |
...
7. **TeamPCP 특화 조치**  
   ...
   - 시스템 프로세스에서 `litellm` 관련 의심스러운 파이썬 프로세스(`ps aux | grep -i litellm`) 검색 및 종료  
   - 쿠버네티스 클러스터에서 비정상적인 서비스·포드(예: `teampcp-backdoor`) 탐지  
...
- **Python 생태계 전반 서명·검증 인프라 확대**: 현재 PyPI는 서명 옵션이 선택적이며, 조직 차원의 강제 적용이 필요함.  
